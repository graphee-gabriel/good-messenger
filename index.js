...
const DELAY_BEFORE_ATTEMPT_STYLE_UPDATE_ONE_SHOT = 100
...
const selectorChatInputChooseASticker =
  '._7mkk a[aria-label="Choose a sticker"]'
const selectorChatInputChooseAFile = '._7mkk form[title="Add files"]'
const selectorChatInputSendALike = 'a[aria-label="Send a Like"]'

const selectorInfoPanelConversationInfoButton =
  'a[aria-label="Conversation Information"]'
const selectorInfoPanelFriendInfoContainer = '._2jnt._6_m-'
const selectorChatInputTextContentContainer =
  '._5rp7._5rp8 ._1mf._1mj > span > span'
...
const setStyleKeysForAllInstancesOfManySelectors = (
  selectors,
  styles,
  delay
) => {
...
const runUpdateStylesLoop = () => {
  ...
  const infoPanelFriendInfoContainer = document.querySelector(
    selectorInfoPanelFriendInfoContainer
  )
  if (infoPanelFriendInfoContainer) {
    if (
      infoPanelFriendInfoContainer.querySelector(
      ...
      infoPanelFriendInfoContainer.appendChild(div)
    }
    const infoPanelFriendInfoContainerCallButtons = infoPanelFriendInfoContainer.querySelector(
      'ul[class="call-buttons"]'
    )
    for (var i = 0; i < 2; i++) {
      const child = document.querySelector('._fl2._6ymr li')
      if (
        child.querySelector('div[aria-label="Start a voice call"]') !== null ||
        child.querySelector('div[aria-label="Start a video chat"]') !== null
      ) {
        infoPanelFriendInfoContainerCallButtons.appendChild(child)
      }
    }
  }
}
...
const runUpdateStylesOneShot = () => {
  const conversationListContainer = document.querySelector(
    selectorConversationListContainer
  )
  ...
  if (conversationListContainer && searchBar && settings && newMessage) {
    ...
    if (updateTrials < 100) {
      updateTrials += 1
      console.log('conversationListContainer not found, trying again', {
        updateTrials,
      })
      setTimeout(
        runUpdateStylesOneShot,
        DELAY_BEFORE_ATTEMPT_STYLE_UPDATE_ONE_SHOT
      )
    } else {
      console.error(
        'conversationListContainer not found, style initialization failed, TOO MANY ATTEMPTS'
      )
    }
  }
}
...
document.addEventListener('keyup', function (event) {
  ...
  const searchContactInputIsFocused =
    searchContactInput && document.activeElement === searchContactInput
  const hasTextInInput =
    document.querySelector(selectorChatInputTextContentContainer) !== null
    ...
    switch (event.key) {
      ...
      case 'ArrowRight': {
        const toggleInfoButton = document.querySelector(
          selectorInfoPanelConversationInfoButton
        )
        if (toggleInfoButton) {
...
setTimeout(runUpdateStylesOneShot, DELAY_BEFORE_ATTEMPT_STYLE_UPDATE_ONE_SHOT)
